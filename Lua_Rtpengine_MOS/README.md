...
**Issue** ERROR: rtpengine [rtpengine.c:1766]: build_rtpp_socks(): Name or service not known
\
**solution** ensure the rtpnegine file in dbtext has correct values and not the default placegholder 
1:1:udp\:<rtpengine_ip>\:2222:1:0:0
```bash
...
```
ofcourse dont forget to reload using cli command
```bash
...
**Issue4**  attempt to concatenate a nil value on mos score of other media stats Pv values
\
...
**Issue5** Server abnormaly exists with build_rtpp_socks(): no more pkg memory for rtpp_socks
```bash
 9(16186) ERROR: rtpengine [rtpengine.c:1722]: build_rtpp_socks(): no more pkg memory for rtpp_socks
 9(16186) ERROR: <core> [core/sr_module.c:847]: init_mod_child(): error while initializing module rtpengine (/usr/local/lib64/kamailio/modules/rtpengine.so) (idx: 9 rank: -2 desc: [Async Task Worker])
 9(16186) ERROR: <core> [core/pt.c:342]: fork_process(): init_child failed for process 9, pid 16186, "Async Task Worker"
 9(16186) ERROR: <core> [main.c:1728]: main_loop(): error in init_child

```
**solution** With a probable bug in source code kamailio version 5.2.1 doesnt connect to an empty rtpengine 
Even if u have an entry in rtpengine file repeat the same twice with a diff id but same set id , like
```bash
id(int,auto) setid(int) url(string) weight(int) disabled(int) stamp(int)
1:1:udp\:192.168.1.109\:2222:1:0:0
2:1:udp\:192.168.1.109\:2222:1:0:0
