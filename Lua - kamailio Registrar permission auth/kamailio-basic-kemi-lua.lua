...
function ksr_route_auth()
	...
	if KSR.pv.get("$rm")=="REGISTER" or KSR.is_myself(KSR.pv.get("$fu")) then
		...
		KSR.info("===== authenticate requests " .. KSR.pv.get("$fd") .. " \n");
		
		--auth_checks - combines the functionalities of www_authenticate and proxy_authenticate
